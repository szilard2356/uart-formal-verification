...
To ensure the hardware met the required specifications, the verification process was executed in three progressive stages:
...
* *Dynamic Verification (Scoreboard): To handle extensive, automated testing (e.g., verifying robustness against baud rate deviations), parallel prediction algorithms were implemented. A Scoreboard component continuously monitored the DUT, comparing the real-time received data (rx_data) against the expected outputs.
